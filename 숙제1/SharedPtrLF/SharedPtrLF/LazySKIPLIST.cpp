...
public:
	...
	LZSKSET() {
		head = new LZSKNode(0x80000000);
		tail = new LZSKNode(0x7FFFFFFF);
		for (int i = 0; i < MAX_LEVEL; i++) {
			head->next[i] = tail;
		}
	}
	...
	int Find(int x, LZSKNode* preds[], LZSKNode* succs[])
	{
		int bottomLevel = 0;
		int lFound = -1;
		bool marked = false;
	...
	bool Add(int x)
	{
		...
		int bottomLevel = 0;
		LZSKNode* preds[MAX_LEVEL];
		LZSKNode* succs[MAX_LEVEL];

		while (true)
		{
			int lFound = Find(x, preds, succs);
			if (lFound != -1) {
				LZSKNode* nodeFound = succs[lFound];
				if (!nodeFound->marked) {
			...
			int highestLocked = -1;
			{
				...
				LZSKNode* newNode = new LZSKNode{ x, topLevel };
